...
*/
class seoaal_sidebar_generator {
	
	public function __construct(){
		add_action('widgets_init',array('seoaal_sidebar_generator','init'), 20);
		add_action('admin_menu',array('seoaal_sidebar_generator','admin_menu'));
		add_action('admin_enqueue_scripts', array('seoaal_sidebar_generator','admin_enqueue_scripts'));
		add_action('admin_print_scripts', array('seoaal_sidebar_generator','admin_print_scripts'));
		if ( current_user_can( 'manage_options' ) ){
			add_action('wp_ajax_add_sidebar', array('seoaal_sidebar_generator','add_sidebar') );
			add_action('wp_ajax_remove_sidebar', array('seoaal_sidebar_generator','remove_sidebar') );
		}
		
		
		//save posts/pages
		add_action('edit_post', array('seoaal_sidebar_generator', 'save_form'));
		add_action('publish_post', array('seoaal_sidebar_generator', 'save_form'));
		add_action('save_post', array('seoaal_sidebar_generator', 'save_form'));
		add_action('edit_page_form', array('seoaal_sidebar_generator', 'save_form'));
	}
	
	public static function init(){
		//go through each sidebar and register it
	    $sidebars = seoaal_sidebar_generator::get_sidebars();
	    
	    if(is_array($sidebars)){
			foreach($sidebars as $sidebar){
				$sidebar_class = seoaal_sidebar_generator::name_to_class($sidebar);
	...
	public static function admin_print_scripts(){
		$ajax_nonce = wp_create_nonce( "seoaal-custom-sidebar" );
		echo '<sc'.'ript>'; 
		?>
	...
	public static function add_sidebar(){
		check_admin_referer( 'seoaal-custom-sidebar', 'sidebar_generator_nonce' );
		$sidebars = seoaal_sidebar_generator::get_sidebars();
		...
		$id = seoaal_sidebar_generator::name_to_class($name);
		if(isset($sidebars[$id])){
			die("alert('Sidebar already exists, please use a different name.')");
		}
		
		$sidebars[$id] = $name;
		seoaal_sidebar_generator::update_sidebars($sidebars);
	...
	public static function admin_menu(){
		add_theme_page('Custom Sidebars', 'Custom Sidebars', 'manage_options', 'multiple_sidebars', array('seoaal_sidebar_generator','admin_page'));
	}
	...
	public static function admin_page(){
		...
		<?php echo '</sc'.'ript>';  ?>
		<div class="wrap">
			<h2><?php esc_html_e( 'Sidebar Generator', 'seoaal' ); ?></h2>
			<p>
				<?php esc_html_e( 'The sidebar name is for your use only. It will not be visible to any of your visitors.', 'seoaal' ); ?> 
				<?php esc_html_e( 'A CSS class is assigned to each of your sidebar, use this styling to customize the sidebars.', 'seoaal' ); ?>
			</p>
			<br />
			<div class="add_sidebar">
				<a href="javascript:void(0);" onclick="return add_sidebar_link()" title="<?php esc_attr_e( 'Add a sidebar', 'seoaal' ); ?>" class="button-primary"><?php esc_html_e( 'Add Sidebar', 'seoaal' ); ?></a>
				<a href="#" title="<?php esc_attr_e( 'Export Sidebars', 'seoaal' ); ?>" class="button-primary export-custom-sidebar"><?php esc_html_e( 'Export Sidebars', 'seoaal' ); ?></a>
			</div>
			<br />
			<table class="widefat page" id="sbg_table">
				<tr>
					<th><?php esc_html_e( 'Sidebar Name', 'seoaal' ); ?></th>
					<th><?php esc_html_e( 'CSS class', 'seoaal' ); ?></th>
					<th><?php esc_html_e( 'Remove', 'seoaal' ); ?></th>
				</tr>
				<?php
				$sidebars = seoaal_sidebar_generator::get_sidebars();
				if(is_array($sidebars) && !empty($sidebars)){
				...
				<tr class="<?php echo esc_attr( $alt );?>">
					...
					<td><?php echo esc_html( seoaal_sidebar_generator::name_to_class($sidebar) ); ?></td>
					<td><a href="javascript:void(0);" onclick="return remove_sidebar_link('<?php echo esc_attr( $sidebar ); ?>',<?php echo esc_attr( $cnt+1 ); ?>);" title="<?php esc_attr_e( 'Remove this sidebar', 'seoaal' ); ?>"><span class="dashicons dashicons-no-alt"></span></a></td>
				</tr>
				<?php
						$cnt++;
					}
					...
					?>
					<tr>
						<td colspan="3"><?php esc_html_e( 'No Sidebars defined', 'seoaal' ); ?></td>
					</tr>
					<?php
				}
				?>
	...
	*/
	public static function save_form($post_id){
		if(isset($_POST['sbg_edit'])){
			...
			if(!empty($is_saving)){
				...
				add_post_meta($post_id, 'sbg_selected_sidebar', $_POST['seoaal_sidebar_generator']);
				add_post_meta($post_id, 'sbg_selected_sidebar_replacement', $_POST['seoaal_sidebar_generator_replacement']);
			}
		}
	}
	...
	<div id='sbg-sortables' class='meta-box-sortables'>
		<div id="sbg_box" class="postbox " >
			<div class="handlediv" title="<?php esc_attr_e( 'Click to toggle', 'seoaal' ); ?>"><br /></div><h3 class='hndle'><span><?php esc_html_e( 'Sidebars', 'seoaal' ); ?></span></h3>
			<div class="inside">
				<div class="sbg_container">
					<input name="sbg_edit" type="hidden" value="sbg_edit" />
					
					<p>
						<span><?php esc_html_e( 'Select the sidebar you wish to display on this page, and which sidebar it will replace. (leave unselected to use the default sidebar everywhere)', 'seoaal' ); ?>
					</p>
					<ul>
					<?php 
					global $wp_registered_sidebars;		
						for($i=0;$i<5;$i++){ ?>
							<li>
							<select name="seoaal_sidebar_generator[<?php echo esc_attr( $i );?>]">
								<option value="0"<?php if($selected_sidebar[$i] == ''){ echo " selected";} ?>><?php esc_html_e( 'WP Default Sidebar', 'seoaal' ); ?></option>
							<?php
							$sidebars = $wp_registered_sidebars;// seoaal_sidebar_generator::get_sidebars();
							if(is_array($sidebars) && !empty($sidebars)){
								foreach($sidebars as $sidebar){
							...
							<select name="seoaal_sidebar_generator_replacement[<?php echo esc_attr( $i );?>]">
								<option value="0"<?php if($selected_sidebar_replacement[$i] == ''){ echo " selected";} ?>><?php esc_html_e( 'None', 'seoaal' ); ?></option>
							<?php
							
							$sidebar_replacements = $wp_registered_sidebars;//seoaal_sidebar_generator::get_sidebars();
	...
	*/
	public static function update_sidebars( $sidebar_array ){
		...
		update_option('seoaal_custom_sidebars', $sidebar_json);
	}	
...
$sbg = new seoaal_sidebar_generator;
?>